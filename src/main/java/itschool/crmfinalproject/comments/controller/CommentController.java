...
public class CommentController {
    ...
    @ApiResponse(responseCode = "200", description = "Successfully retrieved all comments", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentDTO.class))})
    ...
    @ApiResponse(responseCode = "200", description = "Successfully retrieved comments for the author", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentDTO.class))})
    ...
    @ApiResponse(responseCode = "200", description = "Successfully retrieved comments for the event", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentBaseDTO.class))})
    public ResponseEntity<?> getCommentByEventId(@PathVariable String eventId) {
    ...
    @ApiResponse(responseCode = "200", description = "Successfully posted a new comment", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentBaseDTO.class))})
    public ResponseEntity<?> addComment(@RequestBody CommentBaseDTO commentDetails) {
        CommentDTO newComment = commentService.addComment(commentDetails);
        return ResponseEntity.ok(newComment);
    }

    @PostMapping("/reply/{parentId}")
    @Operation(summary = "Add a reply to a comment", description = "Post a reply to an existing comment.")
    @ApiResponse(responseCode = "200", description = "Successfully added a reply to the comment", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentBaseDTO.class))})
    public ResponseEntity<?> replyToComment(@PathVariable String parentId, @RequestBody CommentBaseDTO replyDetails) {
        CommentDTO reply = commentService.replyToComment(parentId, replyDetails);
    ...
    @ApiResponse(responseCode = "200", description = "Successfully added a like to the comment", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = CommentBaseDTO.class))})
    public ResponseEntity<?> addLike(@PathVariable @Parameter(description = "Comment ID to add a like to") String commentId, @PathVariable @Parameter(description = "User ID of the user liking the comment") String userId) {
    ...
    public ResponseEntity<?> removeLike(@PathVariable @Parameter(description = "Comment ID to remove a like from") String commentId, @PathVariable @Parameter(description = "User ID of the user removing the like") String userId) {
